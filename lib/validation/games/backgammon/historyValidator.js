...
export function validateBackgammonGame(submission) {
  const { moveHistory, score, winType, cubeValue, difficulty, loserPipCount = 0 } = submission;
  ...
  // Validation 6: Calculate expected score (including pip bonus)
  const winTypeValue = {
    normal: 1,
    gammon: 2,
    backgammon: 3
  ...
  // Calculate pip bonus (same formula as game)
  ...
  const expectedScore = Math.round(winTypeValue * cubeValue * difficultyValue * pipBonus);

  if (expectedScore !== score) {
    return {
      valid: false,
      reason: `Score mismatch: expected ${expectedScore} (${winType}×${cubeValue}×${difficultyValue}×${pipBonus.toFixed(2)}), got ${score}`,
